...
def load_documents():
    ...
    with st.spinner("Loading and indexing documents..."):
        ...
        st.success(f"✓ Loaded {len(documents)} document chunks from {len(st.session_state.index_stats['sources'])} sources")
        return True
...
def format_response(response):
    ...
    if response.sources:
        ...
        for i, source in enumerate(response.sources, 1):
            with st.expander(f"Source {i}: {Path(source['file']).name} (Similarity: {source['similarity']})"):
...
if not st.session_state.documents_loaded:
    ...
    with tab1:
        ...
        st.markdown("Ask questions about your documents. The system will retrieve relevant passages and answer based on the content.")
    ...
    with tab3:
        ...
        st.markdown("Describe what data you want, and the system will generate SQL queries based on your schema documentation.")
...
st.markdown("---")
st.markdown("<p style='text-align: center; color: gray;'>RAG System v1.0 | Powered by Claude + FAISS</p>", unsafe_allow_html=True)
